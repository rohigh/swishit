...
export default function ProductReviews({ productName = 'Swishit Dishwashing Dew' }) {
  ...
  return (
    <div className="w-full py-8 text-text">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#155E78]/10 shadow-[0_4px_24px_-8px_rgba(0,0,0,0.04)] mb-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-12 gap-6 md:gap-8 items-center">
          
          {/* Average Score Box */}
          <div className="sm:col-span-2 md:col-span-4 text-center md:text-left md:border-r border-[#155E78]/10 md:pr-8">
            <div className="flex items-baseline justify-center md:justify-start gap-2 mb-2">
              <span className="text-5xl font-heading font-bold text-text">4.9</span>
              <span className="text-lg font-medium text-text/60">/ 5.0</span>
            </div>
            ...
            <p className="text-xs text-text/70 font-medium">Based on 2,500+ verified customer reviews</p>
            
            <div className="mt-4 pt-4 border-t border-[#155E78]/10 flex items-center justify-center md:justify-start gap-2 text-xs font-semibold text-green-700">
          ...
          <div className="sm:col-span-2 md:col-span-5 space-y-2">
            {[
              ...
              <div key={item.stars} className="flex items-center gap-3 text-xs">
                <span className="w-8 font-semibold font-mono text-text">{item.stars}</span>
                ...
                <span className="w-10 text-right font-mono text-text/60">{item.percent}%</span>
              </div>
            ))}
          </div>
          ...
          <div className="sm:col-span-2 md:col-span-3 text-center">
            <button
              ...
              className="w-full py-3.5 px-6 rounded-full bg-[#155E78] hover:bg-[#155E78] text-white text-xs font-bold uppercase tracking-wider transition-all shadow-md"
            >
        ...
        {showReviewForm && (
          ...
          >
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#155E78]/20 shadow-lg">
              ...
              <p className="text-xs text-text/70 mb-6">
              ...
              {formSubmitted ? (
                ...
                <form onSubmit={handleSubmitReview} className="space-y-4 max-w-2xl">
                  <div>
                    <label className="block text-xs font-semibold text-text mb-2">Overall Rating</label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-4 py-2.5 rounded-xl border border-gray-300 text-sm text-text focus:outline-none focus:ring-2 focus:ring-[#5AB8D6] bg-white"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-text mb-1">Review Headline</label>
                      <input
                        type="text"
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-text mb-1">Your Review</label>
                    <textarea
                      required
                      rows={4}
                  ...
                  <button
                    type="submit"
                    className="px-8 py-3 rounded-full bg-[#155E78] text-white text-xs font-bold uppercase tracking-wider hover:bg-[#155E78] transition-all shadow-md"
                  >
                    Submit Review
                  </button>
                </form>
      ...
      <div className="space-y-4">
        ...
        {reviews.map((rev) => (
          <div
            key={rev.id}
            className="p-4 sm:p-5 md:p-6 rounded-3xl bg-white border border-[#155E78]/10 shadow-[0_2px_12px_-4px_rgba(0,0,0,0.03)]"
          >
            <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
              ...
              <span className="text-xs text-text/50 font-mono">{rev.date}</span>
            </div>
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <h4 className="font-semibold text-sm text-text">{rev.title}</h4>
            </div>

            <p className="text-sm text-text/80 leading-relaxed font-body">
              {rev.comment}
            </p>
          </div>
        ))}
